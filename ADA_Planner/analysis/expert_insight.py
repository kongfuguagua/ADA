...
在 ADA_Planner 循环启动时，强制执行一次专家诊断，将诊断结果作为强提示注入给 LLM。
"""
...
class ExpertInsightService:
    ...
    def _analyze_sensitivity(
        ...
        try:
            ...
            return {
                ...
                "cost_estimation": {
                    "type": "redispatch", 
                    "estimated_cost": 0.1,  # 再调度成本较低（操作简单，风险小）
                    "cost_breakdown": {
                        "operation_cost": 0.05,
                        "risk_cost": 0.0,  # 再调度风险很小
                        "operational_efficiency": 0.05  # 可能带来运营效率提升
                    }
                },
    ...
    def _estimate_cost(
        ...
        if 'set_bus' in action_dict and action_dict['set_bus'] is not None:
            ...
            # 如果评分高（3-4），降低基础成本（因为更安全、更有效）
            if score >= 3:
                base_cost *= 0.8  # 高评分方案成本更低
            
            cost["estimated_cost"] = base_cost
            cost["cost_breakdown"] = {
                "operation_cost": 0.5,
                "risk_cost": 0.5 if score <= 2 else 0.0,
                "efficiency_bonus": -0.2 if score >= 3 else 0.0
            }
        ...
        # 3. 尝试通过模拟估算运营成本（如果可能）
        try:
            # 注意：这里不进行实际模拟（因为已经在 expert_operator 中模拟过了）
            # 但我们可以基于 score 和副作用来估算
            # 高评分（3-4）通常意味着更好的负载分布，运营成本更低
            if score >= 3:
                cost["estimated_cost"] -= 0.1  # 运营效率奖励
                cost["cost_breakdown"]["operational_efficiency"] = -0.1
        except:
            pass
        
        # 确保成本不为负
        cost["estimated_cost"] = max(0.0, cost["estimated_cost"])
