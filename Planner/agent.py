# -*- coding: utf-8 -*-
"""
Planner Agent 核心实现 (Fix: Logic Bug & Auto-Correction)
"""
...
try:
    ...
    from Planner.formatters import ObservationFormatter
    from Planner.parser import ActionParser
    from Planner.prompts import PromptManager
...
try:
    ...
    try:
        from Planner.analysis import ExpertInsightService
        _HAS_EXPERT_INSIGHT = True
    except ImportError:
...
class Planner(BaseAgent):
    """
    Planner Baseline Agent (Enhanced with Expert Insight)
    ...
    def __init__(
        ...
        **kwargs
        ...
        logger.info(f"Planner '{name}' 初始化完成 (v3.0 Expert-Augmented)")
    ...
    def _format_expert_insight(self, insight_report: Dict[str, Any]) -> str:
        ...
        for i, solution in enumerate(solutions):
            ...
            cost_str = f", Cost: {cost.get('estimated_cost', 0.0):.2f}" if cost else ""
            ...
            if sol_type == "Topology Action":
                ...
                lines.append(f"[{i}] Redispatch {gen_str}")
