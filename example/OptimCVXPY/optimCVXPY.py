...
class OptimCVXPY(BaseAgent):
    ...
    def _update_topo_param(self, obs: BaseObservation):
        ...
        # Ensure integer type after all assignments
    ...
    def _aux_compute_kcl(self, inj_bus, f_or):
        KCL_eq = []
        # Convert to integers for comparison and indexing
    ...
    def compute_optimum_safe(self, obs: BaseObservation, l_id=None):
        if l_id is not None:
            ...
            self.bus_ex.value[l_id] = int(obs.line_ex_to_subid[l_id])
            self.bus_or.value[l_id] = int(obs.line_or_to_subid[l_id])
